...
const askGemini = async (historial, username) => {
  const contents = [
    {
      role: 'model',
      parts: [{ text: getPrompt(username) }] // 👈 nombre ya va dentro del prompt
    },
    {
      role: 'user',
      parts: [{ text: historial.at(-1)?.text || 'Hola' }] // solo último mensaje
    }
  ];
